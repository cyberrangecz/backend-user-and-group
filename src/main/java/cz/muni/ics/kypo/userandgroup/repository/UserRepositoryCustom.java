...
public interface UserRepositoryCustom {
     ...
     * @return returns list of all {@link User}s except those in {@link cz.muni.ics.kypo.userandgroup.domain.IDMGroup} with given ID wrapped in {@link Page}
     */
    Page<User> usersNotInGivenGroup(@Param("groupId") Long groupId, Predicate predicate, Pageable pageable);

    /**
     * Find all users in given {@link cz.muni.ics.kypo.userandgroup.domain.IDMGroup}s with given IDs.
     *
     * @param groupsIds unique identifiers of groups whose users will be included in the resulting list
     * @param predicate represents a predicate (boolean-valued function) of one argument.
     * @param pageable  abstract interface for pagination information
     * @return returns list of all {@link User}s who are in {@link cz.muni.ics.kypo.userandgroup.domain.IDMGroup} with given ID wrapped up in {@link Page}
     */
